...
# ── Version manifest (auto-update) ───────────────────────────────────
# Stored under Redis key ``version:manifest`` so the seller can update it
# without redeploying the license server. The desktop tool's updater
# polls /api/version and acts on whatever the admin set here.
_DEFAULT_VERSION_MANIFEST = {
    "version":       "2.0.0",
    "url":           "",
    "sha256":        "",
    "min_supported": "2.0.0",
...
def get_version_manifest(r) -> dict:
    raw = r.get("version:manifest")
    if not raw:
        return dict(_DEFAULT_VERSION_MANIFEST)
    try:
        data = json.loads(raw) if isinstance(raw, str) else raw
        merged = dict(_DEFAULT_VERSION_MANIFEST)
...
def save_version_manifest(r, manifest: dict) -> None:
    r.set("version:manifest", json.dumps(manifest))


@app.route("/api/version", methods=["GET", "OPTIONS"])
def api_version():
    """Public manifest the desktop EXE polls for auto-updates.

    Returns a JSON document::

        { "version": "2.1.0",
          "url":     "https://github.com/.../ColdNerd.exe",
          "sha256":  "abc123…",
          "notes":   "What's new",
          "mandatory": false }
    """
    ...
    try:
        r = get_redis()
        m = get_version_manifest(r)
    except Exception:
        m = dict(_DEFAULT_VERSION_MANIFEST)
    return cors(m, 200)


@app.route("/api/admin/version", methods=["POST", "OPTIONS"])
def admin_set_version():
    """Admin-only: replace the auto-update manifest.

    Body fields: version, url, sha256, min_supported, notes, mandatory.
    Any field omitted falls back to the existing value.
    """
    if request.method == "OPTIONS":
        return cors({}, 200)
    ...
    r = get_redis()
    cur = get_version_manifest(r)
    for k in ("version", "url", "sha256", "min_supported", "notes"):
        if k in d and d[k] is not None:
            cur[k] = str(d[k]).strip()
    if "mandatory" in d:
        cur["mandatory"] = bool(d["mandatory"])
    if not cur.get("version") or not cur.get("url"):
        return cors({"success": False, "error": "version and url are required"}, 400)
    save_version_manifest(r, cur)
    return cors({"success": True, "manifest": cur})
...
# ── Per-license lifetime activity counters ─────────────────────────────
# These are independent of the admin-capped quotas (leads_limit) and only
# ever go up. They power the admin dashboard's "Scraped" / "AI Scraped" /
# "Messages Generated" columns so the seller can see total work performed
# under each key, even after they top up the quota.
#
# Redis keys:
#   usage:{key}:scrapes_lifetime       -> total profiles scraped
#   usage:{key}:ai_scrapes_lifetime    -> total profiles AI-classified
#   usage:{key}:messages_lifetime      -> total messages generated by Msg Gen

def _get_lifetime_counter(r, key: str, suffix: str) -> int:
    try:
        v = r.get(f"usage:{key}:{suffix}")
        return int(v) if v else 0
    except Exception:
        return 0


def _bump_lifetime_counter(r, key: str, suffix: str, count: int) -> int:
    """Atomic incrby for a per-license lifetime counter; returns new value."""
    if count <= 0:
        return _get_lifetime_counter(r, key, suffix)
    try:
        new_val = r.incrby(f"usage:{key}:{suffix}", int(count))
        return int(new_val) if new_val else 0
    except Exception:
        return _get_lifetime_counter(r, key, suffix)


@app.route("/api/usage/activity", methods=["POST", "OPTIONS"])
def usage_activity():
    """Increment one or more per-license lifetime activity counters.

    Body::

        { "key": "...",
          "scrapes":     <int>,   # profiles scraped (raw)
          "ai_scrapes":  <int>,   # profiles AI-classified
          "messages":    <int> }  # comments/DMs generated by Msg Gen

    Any counter omitted (or <= 0) is left untouched. Returns the new
    lifetime totals for all three so the client can update its UI.
    """
    if request.method == "OPTIONS":
        return cors({}, 200)
    ...
    scrapes    = int(d.get("scrapes")    or 0)
    ai_scrapes = int(d.get("ai_scrapes") or 0)
    messages   = int(d.get("messages")   or 0)

    new_scrapes    = _bump_lifetime_counter(r, key, "scrapes_lifetime",    scrapes)
    new_ai_scrapes = _bump_lifetime_counter(r, key, "ai_scrapes_lifetime", ai_scrapes)
    new_messages   = _bump_lifetime_counter(r, key, "messages_lifetime",   messages)

    return cors({
        "success": True,
        "scrapes_lifetime":    new_scrapes,
        "ai_scrapes_lifetime": new_ai_scrapes,
        "messages_lifetime":   new_messages,
    })
...
def admin_list_keys():
    ...
    for key in all_keys:
        ...
        # Lifetime activity counters (scrape / AI scrape / message generation).
        scrapes_lt    = _get_lifetime_counter(r, key, "scrapes_lifetime")
        ai_scrapes_lt = _get_lifetime_counter(r, key, "ai_scrapes_lifetime")
        messages_lt   = _get_lifetime_counter(r, key, "messages_lifetime")
        keys_data.append({
            ...
            # NEW lifetime activity counters
            "scrapes_lifetime":     scrapes_lt,
            "ai_scrapes_lifetime":  ai_scrapes_lt,
            "messages_lifetime":    messages_lt,
        })
